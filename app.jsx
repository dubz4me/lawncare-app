...
{section === "accounting" && (
<><section><h2 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--accent)" }}>Summary</h2><div className="flex rounded-lg border border-gray-300 overflow-hidden text-sm mb-3"><button onClick={() => setSummaryPeriod("month")} className="flex-1 py-2"
...
{justCreated && (
<div className="border rounded-lg p-3 text-xs" style={{ borderColor: "var(--success)", backgroundColor: "#ECFDF5" }}><p className="font-medium text-gray-800 mb-1">{justCreated.role === "manager" ? "Manager" : "Crew"} login created — share these with {justCreated.name || justCreated.username} now.</p><p className="text-gray-600">This password won't be shown again after you leave this screen.</p><div className="bg-white border border-gray-200 rounded-md p-2 mt-2 font-mono text-gray-800"><div>Username: {justCreated.username}</div><div>Password: {justCreated.password}</div></div><button onClick={copyCredentials} className="w-full rounded-lg py-2 text-xs font-medium mt-2 text-white" style={{ backgroundColor: ACCENT_HEX }}>
...
(async () => {
try {
const res = await fetch(`${API_BASE}/api/crew-profiles/${encodeURIComponent(profileKey)}/photo${version ? `?v=${version}` : ""}`, { headers: authHeaders() });
if (res.ok) {
objectUrl = URL.createObjectURL(await res.blob());
if (!cancelled) setSrc(objectUrl);
} else if (!cancelled) setSrc(null);
} catch (e) { if (!cancelled) setSrc(null); }
})();
return () => { cancelled = true; if (objectUrl) URL.revokeObjectURL(objectUrl); };
}, [profileKey, version]);
const initials = String(name || "?").split(/\s+/).filter(Boolean).slice(0,2).map((x) => x[0]).join("").toUpperCase();
return (
<div className="rounded-2xl overflow-hidden flex items-center justify-center shrink-0 font-bold"
style={{ width: size, height: size, background: "linear-gradient(135deg,#EEE7F5,#EAF8E7)", color: "var(--accent)" }}>
{src ? <img src={src} alt={`${name || "Team member"} profile`} className="w-full h-full object-cover" /> : <span style={{ fontSize: Math.max(13, size * .28) }}>{initials}</span>}
</div>
);
}
...
const [editing, setEditing] = useState(false);
const [draft, setDraft] = useState({ bio: "", phone: "", email: "", address: "", emergencyName: "", emergencyPhone: "" });
const [saving, setSaving] = useState(false);
const [message, setMessage] = useState("");
...
<><div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-sm"><div className="flex items-start gap-4"><CrewProfileAvatar profileKey={selectedProfileKey} name={detail.name} size={78} version={detail.photoUpdatedAt} /><div className="min-w-0 flex-1"><p className="text-xl font-bold text-gray-900">{detail.name || "Team Member"}</p><p className="text-sm text-gray-500">R-DUB'S Lawn Care Team</p><span className="inline-block mt-2 text-xs font-bold rounded-full px-2.5 py-1" style={{ background: "var(--surface-alt)", color: "var(--accent)" }}>
...
className="w-full bg-white border border-gray-200 rounded-2xl p-3 shadow-sm flex items-center gap-3 text-left"><CrewProfileAvatar profileKey={p.profileKey} name={p.name} size={58} version={p.photoUpdatedAt} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="text-sm font-bold text-gray-900 truncate">{p.name || "Team Member"}</p><span className="text-xs font-semibold rounded-full px-2 py-0.5" style={{ background: "var(--surface-alt)", color: "var(--accent)" }}>{p.role}</span></div><p className="text-xs text-gray-500">R-DUB'S Lawn Care Team</p><p className="text-xs text-gray-600 mt-1 truncate">{p.bio || (p.isSelf ? "Tap to set up your profile." : "No work bio yet.")}</p></div><ArrowRight size={16} className="text-gray-400" /></button>
))}
</div>
)}
</div></div>
...
<div className={`${enlargeMode ? "app-enlarged " : ""}app-shell max-w-md mx-auto min-h-screen bg-white`} style={(darkModeLoaded && enlargeLoaded) ? undefined : { visibility: "hidden" }}><div className="flex items-center justify-between border-b border-gray-200 sticky top-0 bg-white z-10 px-1"><div className="flex items-center">
...
</div><button onClick={() => setLayoutEditMode((v) => !v)} aria-label="Customize navigation"
className="shrink-0 px-2.5 py-2.5" style={{ color: layoutEditMode ? "var(--accent)" : "var(--text-faint)" }}><Settings2 size={15} /></button></div>
{mode === "home" && (
<RoleDashboard
authUser={authUser}
