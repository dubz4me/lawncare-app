...
// Inlined icon set — matches the ~18 Lucide icons this app uses. Kept as
// plain inline SVGs rather than an external icon library dependency, since
// this is now a real standalone page and these icons never change.
...
const API_BASE = "https://rdubs-lawncare-api.rdubslawncare.workers.dev";

// Mirrors window.storage's async, {value}-wrapped API but backs onto real
// localStorage — for settings that are genuinely per-device (auth token,
// dark mode, nav layout, active timer, today's route order), not business
// data shared across the crew. window.storage only exists inside Claude's
// own chat/artifact preview; it does not exist on a real deployed page.
const localStore = {
  async get(key) {
    try {
...
// Default hours match a common pattern: shorter weekdays, longer weekends.
// Fully editable per day in the Business tab.
...
// Breaks a day's custom start–end window into sequential 1-hour bookable blocks,
// each followed by a 30-minute rest gap before the next one. Customers only ever
// see the 1-hour block itself — the rest gap is just internal breathing room.
...
// Fetches many keys with bounded concurrency (a handful at a time) instead of
// firing every request at once — a business with real history can easily have
// 50-100+ keys in one list, and storage requests are rate limited, so an
// unbounded Promise.all risks some calls failing silently.
// Auth token lives in one module-level place so the data-layer functions
// below can reach it without every single call site needing to pass it
// through. App sets this on login/logout via setCurrentAuthToken.
...
function snakeToCamel(str) {
  ...
  // dateISO is written with a capital acronym everywhere else in this app —
  // a plain generic converter would produce "dateIso" and silently break
  // every dateISO reference throughout scheduling.
  return camel.replace(/Iso\b/, "ISO");
}

// Converts one raw SQL row (snake_case, JSON-as-text, 0/1 booleans) into
// the camelCase shape with real objects/booleans that the rest of the app
// has always expected — this is the one place that difference is bridged.
...
// Outgoing (app -> API) needs no conversion: the Worker already accepts
// camelCase field names and converts booleans to 0/1 itself on insert.
...
// Loads a whole table and returns it keyed by its id field, matching the
// shape every existing caller already expects (Object.values, table[id], ...).
...
// Upserts every record currently in the given table object. Used both for
// single-record saves (via setTableRecord) and for the handful of bulk
// operations that modify several records in memory before persisting —
// those pass the whole modified table here rather than one record.
...
// Photos are deliberately NOT part of the consolidated table system — a table
// is one shared blob, and a handful of large images inside it would bloat
// every single read/write of that table for everyone, recreating the exact
// problem the consolidation fixed. Photos get their own key per job instead,
// loaded only when actually viewed, and compressed client-side first since a
// modern phone photo is easily several MB straight out of the camera and
// this storage caps values at 5MB.
...
function BusinessView({ onEditRequest, onPrefillEstimate, introEnabled, onToggleIntro, userRole }) {
  const [motivationLine] = useState(() => randomMotivationLine());
  ...
  async function handlePhotoUpload(item, file, kind) {
    ...
    try {
      ...
      await localStore.set(storageKey, dataUri, false);
      ...
      setPhotoError("Something went wrong saving that photo — try again.");
    } finally {
  ...
  async function viewPhoto(item, kind) {
    ...
    try {
      const r = await localStore.get(storageKey, false);
      if (r && r.value) setViewingPhotoDataUri(r.value);
    } catch (e) {}
  }
  ...
  async function deletePhoto(item, kind) {
    ...
    const flagField = kind === "before" ? "hasBeforePhoto" : "hasPhoto";
    try {
      await localStore.delete(storageKey, false);
  ...
  return (
    <div style={{ paddingBottom: 32 + BOTTOM_TAB_HEIGHT }}>
      ...
      <div className="px-4 py-4 space-y-6">
        ...
        <section>
          <label className="flex items-center justify-between text-xs text-gray-500 border border-gray-200 rounded-lg px-3 py-2 cursor-pointer">
            <span>Intro animation on app open</span>
            <input type="checkbox" checked={introEnabled} onChange={onToggleIntro} style={{ accentColor: "var(--accent)", touchAction: "pan-y" }} />
          </label>
        </section>
      </div>
              ...
              <div className="border border-gray-200 rounded-lg p-3 mb-3 space-y-2.5">
                ...
                {pendingDetailItem.status === "completed" && (
                  <div className="pt-2.5 border-t border-gray-100">
                    <p className="text-xs font-medium text-gray-700 mb-1.5">After photo</p>
                    {pendingDetailItem.hasPhoto ? (
                      <div className="flex gap-2">
                        <button onClick={() => viewPhoto(pendingDetailItem, "after")}
                          className="text-xs rounded-full px-3 py-1.5 border" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>
                          View photo
                        </button>
                        <label className="text-xs rounded-full px-3 py-1.5 border border-gray-300 text-gray-600 cursor-pointer">
                          {photoUploading === `after:${pendingDetailItem.timestamp}` ? "Saving…" : "Replace"}
                          <input type="file" accept="image/*" capture="environment" className="hidden"
                            onChange={(e) => handlePhotoUpload(pendingDetailItem, e.target.files && e.target.files[0], "after")} />
                        </label>
                      </div>
                    ) : (
                      <label className="inline-block text-xs rounded-full px-3 py-1.5 border cursor-pointer" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>
                        {photoUploading === `after:${pendingDetailItem.timestamp}` ? "Saving…" : "+ Add after photo"}
                        <input type="file" accept="image/*" capture="environment" className="hidden"
                          onChange={(e) => handlePhotoUpload(pendingDetailItem, e.target.files && e.target.files[0], "after")} />
                      </label>
                    )}
...
function CustomerDirectoryView({ onEditVisit, initialExpandedPhone, onConsumeInitialExpandedPhone }) {
  ...
  // Protects customers whose estimates predate the auto-profile fix: give
  // anyone with job history but no profile record a profile now, so a future
  // deletion of their last job won't erase them from the directory.
  ...
  async function confirmDeleteCustomer() {
    ...
    try {
      ...
      // leave the confirmation open so they can retry
    } finally {
...
function App() {
  ...
  const [utilityOrder, setUtilityOrder] = useState(["prices", "directory", "books", "darkmode", "logout"]);
  ...
  useEffect(() => {
    ...
    (async () => {
      let enabled = true;
      try {
        const r = await localStore.get("settings:intro-enabled", false);
    ...
    (async () => {
      try {
        ...
        if (r && r.value) {
          ...
          if (Array.isArray(parsed.utilityOrder) && parsed.utilityOrder.length === 5) setUtilityOrder(parsed.utilityOrder);
        }
      } catch (e) {}
    })();
    (async () => {
      try {
        ...
        // network hiccup checking the token — treat as logged out rather than get stuck
      } finally {
  ...
  async function toggleIntroEnabled() {
    const next = !introEnabled;
    setIntroEnabled(next);
    try {
      await localStore.set("settings:intro-enabled", String(next), false);
    } catch (e) {}
  }
  ...
  const UTILITY_DEFS = {
    prices: {
      ...
      onClick: () => { setPricesDrawerOpen((v) => !v); setCustomerDrawerOpen(false); setBooksDrawerOpen(false); },
      active: pricesDrawerOpen,
    },
    directory: {
      label: "Customer Directory", icon: Users,
      onClick: () => { setCustomerDrawerOpen((v) => !v); setBooksDrawerOpen(false); setPricesDrawerOpen(false); },
      active: customerDrawerOpen,
    },
    books: {
      label: "Books", icon: BookOpen,
      onClick: () => { setBooksDrawerOpen((v) => !v); setCustomerDrawerOpen(false); setPricesDrawerOpen(false); },
      ...
      <div className="max-w-md mx-auto min-h-screen bg-white" style={darkModeLoaded ? undefined : { visibility: "hidden" }}>
        ...
        {mode === "business" && (
          <BusinessView onEditRequest={(item) => { setEditRequest(item); setMode("newcustomer"); }}
            ...
            introEnabled={introEnabled} onToggleIntro={toggleIntroEnabled}
