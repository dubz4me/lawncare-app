...
function App() {
  ...
  const [utilityOrder, setUtilityOrder] = useState(["prices", "directory", "books", "bugs", "darkmode", "logout"]);
    ...
    (async () => {
      try {
        ...
        if (r && r.value) {
          ...
          if (Array.isArray(parsed.utilityOrder) && parsed.utilityOrder.length === 6) setUtilityOrder(parsed.utilityOrder);
        }
      } catch (e) {}
    })();
    (async () => {
  ...
  const UTILITY_DEFS = {
    prices: {
      ...
      onClick: () => { setPricesDrawerOpen((v) => !v); setCustomerDrawerOpen(false); setBooksDrawerOpen(false); setBugsDrawerOpen(false); },
      active: pricesDrawerOpen,
    },
    directory: {
      label: "Customer Directory", icon: Users,
      onClick: () => { setCustomerDrawerOpen((v) => !v); setBooksDrawerOpen(false); setPricesDrawerOpen(false); setBugsDrawerOpen(false); },
      active: customerDrawerOpen,
    },
    books: {
      label: "Books", icon: BookOpen,
      onClick: () => { setBooksDrawerOpen((v) => !v); setCustomerDrawerOpen(false); setPricesDrawerOpen(false); setBugsDrawerOpen(false); },
      active: booksDrawerOpen,
      hidden: authUser && authUser.role !== "owner",
    },
    bugs: {
      label: "Report a Bug", icon: Bug,
      onClick: () => { setBugsDrawerOpen((v) => !v); setCustomerDrawerOpen(false); setPricesDrawerOpen(false); setBooksDrawerOpen(false); },
