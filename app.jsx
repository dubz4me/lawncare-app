...
/* Hide the bottom Home / New Customer / Jobs bar while any pop-up is open. Pop-ups opened
   from inside a side panel sit on that panel's lower layer, so the bar would cover them. */
...
/* color-scheme: dark already draws light picker icons; an extra invert filter would turn them dark again */
...
// The server identifies each record by its id field (id for bookings and appointments,
// timestamp for jobs, phone for customers). Callers pass that id separately, so make
// sure it is in the body — without it the server rejects the save (this is why
// bookings and appointments never saved before).
...
<label className="flex items-center gap-1.5 w-16 shrink-0 text-gray-800 font-medium">
...
<div className="mb-2">
<p className="text-xs font-semibold text-gray-900">After photos</p>
...
<section>
<h2 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--accent)" }}>Accounting Exports</h2>
<div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-3">
<div>
<p className="text-sm text-gray-900 font-bold">Jobs & revenue</p>
<p className="text-xs text-gray-500 mb-2">For Excel, Google Sheets, or your own reporting.</p>
<button onClick={exportJobsCSV} className="text-xs rounded-xl px-3 py-2 border font-semibold" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>
Download jobs (.csv)
</button>
</div>
<div className="border-t border-gray-200 pt-3">
<p className="text-sm text-gray-900 font-bold">Expenses</p>
<p className="text-xs text-gray-500 mb-2">A clean expense file for bookkeeping or tax prep.</p>
<button onClick={exportExpensesCSV} className="text-xs rounded-xl px-3 py-2 border font-semibold" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>
...
{section === "backup" && (
<section>
<h2 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--accent)" }}>Data & Backup</h2>
<div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-4">
<div>
<p className="text-sm text-gray-900 font-bold">Download full backup</p>
<p className="text-xs text-gray-500 mb-2">Customers, jobs, appointments, expenses, inventory, crew schedules, and settings in one file.</p>
...
<p className="text-sm text-gray-900 font-bold">Restore from backup</p>
<p className="text-xs text-gray-500 mb-2">Imports an R-DUB'S backup and updates matching records without deleting the rest.</p>
<label className="inline-block text-xs rounded-xl px-3 py-2 border cursor-pointer font-semibold" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>
...
<div>
<p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--accent)" }}>R-DUB'S Lawn Care</p>
<p className="text-xl font-bold text-gray-900 leading-tight mt-0.5">Team Management</p>
<p className="text-sm text-gray-600 mt-0.5">Accounts, roles, access, passwords, and team controls.</p>
</div>
</div>
</div>
<div className="px-4 py-4 space-y-5">
...
<button onClick={() => setAddMemberOpen((v) => !v)} className="w-full flex items-center justify-between gap-3 p-4 text-left">
<div className="flex items-center gap-3">
<div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "var(--surface-alt)", color: "var(--accent)" }}><UserPlus size={19} /></div>
<div>
<p className="text-sm font-bold text-gray-900">Add a Team Member</p>
<p className="text-xs text-gray-500">Create a Crew or Manager login.</p>
</div>
</div>
...
className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm mb-3" />
<label className="block text-xs font-semibold text-gray-600 mb-1">Username</label>
<input type="text" value={newUsername} onChange={(e) => setNewUsername(e.target.value)} placeholder="e.g. mike"
className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-sm mb-3" autoCapitalize="none" autoCorrect="off" />
<label className="block text-xs font-semibold text-gray-600 mb-1">Role</label>
<div className="grid grid-cols-2 gap-2 mb-3">
<button onClick={() => setNewRole("crew")} className="rounded-xl py-2.5 text-xs font-bold border"
style={newRole === "crew" ? { borderColor: "var(--accent)", backgroundColor: "var(--surface-alt)", color: "var(--accent)" } : { borderColor: "#D1D5DB", color: "#6B7280" }}>Crew</button>
<button onClick={() => setNewRole("manager")} className="rounded-xl py-2.5 text-xs font-bold border"
style={newRole === "manager" ? { borderColor: "var(--accent)", backgroundColor: "var(--surface-alt)", color: "var(--accent)" } : { borderColor: "#D1D5DB", color: "#6B7280" }}>Manager</button>
</div>
<label className="block text-xs font-semibold text-gray-600 mb-1">Generated password</label>
<div className="flex items-center gap-2">
<input type="text" value={newPassword} readOnly className="flex-1 border border-gray-300 rounded-xl px-3 py-2.5 text-sm font-mono bg-gray-50" />
<button onClick={() => setNewPassword(generateOnboardPassword())} className="text-xs px-3 py-2.5 border border-gray-300 rounded-xl text-gray-600">New</button>
</div>
...
<div>
<p className="text-base font-bold text-gray-900">Team Access</p>
<p className="text-xs text-gray-500 mt-0.5">{users.length} accounts · {crewCount} crew · {managerCount} managers</p>
</div>
</div>
<div className="bg-white border border-gray-200 rounded-2xl p-3 shadow-sm">
<div className="relative">
<input type="search" value={teamSearch} onChange={(e) => setTeamSearch(e.target.value)}
...
<div className="space-y-2">
{filteredUsers.map((u) => (
<div key={u.username} className="bg-white border border-gray-200 rounded-2xl p-4 text-xs shadow-sm">
<div className="flex items-center justify-between">
<div>
...
<p className="text-sm font-bold text-gray-900 mt-2">No matching team members</p>
<p className="text-xs text-gray-500 mt-1">Try a different name or role filter.</p>
</div>
)}
</div>
)}
...
<section className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
<button onClick={() => setDeletedOpen((v) => !v)} className="w-full flex items-center justify-between p-4 text-left">
<div>
<p className="text-sm font-bold text-gray-900">Recently Deleted Customers</p>
<p className="text-xs text-gray-500 mt-0.5">Restore accidentally deleted customer records.</p>
</div>
...
function PhotoChecklistModal({ item, phase, userRole, onClose, onFlagsChanged, onViewLegacy }) {
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-center" style={{ zIndex: 60 }} onClick={(e) => e.stopPropagation()}>
      ...
      <div className="relative bg-white w-full max-w-md rounded-t-3xl overflow-y-auto modal-panel shadow-2xl" style={{ maxHeight: "92vh" }}>
        <div className="px-4 pt-4 pb-4 border-b border-gray-200" style={{ background: "linear-gradient(135deg,#F7F3FA 0%,#FFFFFF 65%,#F1FBEF 100%)" }}>
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0" style={{ background: "rgba(75,46,112,.10)", color: "var(--accent)" }}>
                <span style={{ fontSize: 22 }}>{phase === "before" ? "📷" : phase === "after" ? "✨" : "⚠️"}</span>
              </div>
              <div className="min-w-0">
                ...
                <p className="text-lg font-bold text-gray-900 leading-tight truncate">{item.customerName || "Job"}</p>
                <p className="text-xs text-gray-500 mt-0.5">{title}</p>
              </div>
            </div>
            <button onClick={onClose} aria-label="Close" className="w-9 h-9 rounded-xl border border-gray-200 bg-white flex items-center justify-center text-gray-500 shrink-0"><X size={18} /></button>
          </div>
        </div>
        <div className="p-4">
        ...
        {data && !finished && current && (
          <div>
            ...
            {phase !== "issue" && (
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1.5">
                  <p className="text-xs font-semibold text-gray-500">Shot {step + 1} of {shots.length}</p>
                  <p className="text-xs font-semibold" style={{ color: "var(--accent)" }}>{done.size} saved</p>
                </div>
                <div className="h-1.5 rounded-full bg-gray-100 overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${Math.min(100, (done.size / Math.max(1, shots.length)) * 100)}%`, backgroundColor: "var(--accent)" }} />
                </div>
              </div>
            )}
            <div className="bg-white border border-gray-200 rounded-2xl p-3 mb-3 shadow-sm">
            ...
            {refUrl && !preview && (
              <div className="mb-3 rounded-2xl border border-purple-100 overflow-hidden" style={{ background: "#F8F5FB" }}>
                <div className="px-3 py-2 flex items-center justify-between">
                  <p className="text-xs font-bold" style={{ color: "var(--accent)" }}>Before-photo reference</p>
                  <span className="text-xs text-gray-500">Match this angle</span>
                </div>
                <button type="button" onClick={() => setEnlargedImage({ src: refUrl, label: "Before-photo reference" })} className="block w-full text-left">
                  <img src={refUrl} alt="Before reference" className="w-full" style={{ maxHeight: 180, objectFit: "cover", opacity: 0.82 }} />
                  <div className="px-3 py-2 text-center text-xs font-semibold" style={{ color: "var(--accent)" }}>Tap to enlarge</div>
                </button>
              </div>
            )}
            ...
            {preview ? (
              <div>
                <button type="button" onClick={() => setEnlargedImage({ src: preview, label: `${current.label} preview` })} className="block w-full mb-3">
                  <img src={preview} alt="Preview" className="w-full rounded-xl" style={{ maxHeight: 280, objectFit: "contain" }} />
                  <p className="text-xs font-semibold mt-1" style={{ color: "var(--accent)" }}>Tap photo to enlarge</p>
                </button>
                {phase === "issue" && (
              ...
              <div>
                {shownUrl
                  ? (
                    <button type="button" onClick={() => setEnlargedImage({ src: shownUrl, label: current.label })} className="block w-full mb-2">
                      <img src={shownUrl} alt={current.label} className="w-full rounded-xl" style={{ maxHeight: 280, objectFit: "contain" }} />
                      <p className="text-xs font-semibold mt-1" style={{ color: "var(--accent)" }}>Tap photo to enlarge</p>
                    </button>
                  )
      ...
      {enlargedImage && (
        <div className="fixed inset-0 z-50 flex flex-col bg-black" onClick={() => setEnlargedImage(null)}>
          <div className="flex items-center justify-between gap-3 px-4 py-3 text-white" style={{ background: "rgba(0,0,0,.82)" }}>
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-wider text-gray-300">Enlarged photo</p>
              <p className="text-sm font-semibold truncate">{enlargedImage.label}</p>
            </div>
            <button type="button" onClick={() => setEnlargedImage(null)}
              className="w-10 h-10 rounded-xl flex items-center justify-center border border-white/30 shrink-0">
              <X size={20} />
            </button>
          </div>
          ...
          <button type="button" onClick={() => setEnlargedImage(null)}
            ...
            style={{ background: "rgba(255,255,255,.10)" }}>Close enlarged view</button>
        </div>
      )}
    </div>
  );
...
function SettingsHubView({ authUser, darkMode, enlargeMode, onToggleDarkMode, onToggleEnlarge, onOpenWebsite, onOpenPassword, onOpenBug, onOpenBackup, onStartTour, onLogout }) {
const owner = authUser && authUser.role === "owner";
const SettingRow = ({ icon: Icon, title, text, onClick, value }) => (
<button onClick={onClick} className="w-full flex items-center gap-3 p-4 text-left bg-white border border-gray-200 rounded-2xl shadow-sm">
<div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: "var(--surface-alt)", color: "var(--accent)" }}>
<Icon size={18} />
</div>
<div className="min-w-0 flex-1">
...
<div>
<p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--accent)" }}>R-DUB'S Lawn Care</p>
<p className="text-xl font-bold text-gray-900 leading-tight mt-0.5">Settings</p>
<p className="text-sm text-gray-600 mt-0.5">App preferences, support, security, and owner tools.</p>
</div>
</div>
</div>
<div className="px-4 py-4 space-y-5">
<section>
<p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: "var(--accent)" }}>App</p>
<div className="space-y-2">
<SettingRow icon={MaximizeIcon} title="App Size" text="Use a wider desktop workspace or the normal compact layout." onClick={onToggleEnlarge} value={enlargeMode ? "Enlarged" : "Normal"} />
<SettingRow icon={darkMode ? Sun : Moon} title="Appearance" text="Switch between light and dark mode." onClick={onToggleDarkMode} value={darkMode ? "Dark" : "Light"} />
<SettingRow icon={HomeIcon} title="Take App Tour" text="Have Gunner walk through the app again." onClick={onStartTour} />
</div>
</section>
<section>
<p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: "var(--accent)" }}>Account & Support</p>
<div className="space-y-2">
<SettingRow icon={LockIcon} title="Change Password" text="Update the password for your current account." onClick={onOpenPassword} />
<SettingRow icon={Bug} title="Report a Bug" text="Tell R-DUB'S what went wrong and where it happened." onClick={onOpenBug} />
</div>
</section>
{owner && (
<section>
<p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: "var(--accent)" }}>Owner Tools</p>
<div className="space-y-2">
<SettingRow icon={Globe} title="Website Settings" text="Edit website content and owner-controlled public settings." onClick={onOpenWebsite} />
<SettingRow icon={BookOpen} title="Data & Backup" text="Download a full backup or restore from one." onClick={onOpenBackup} />
</div>
</section>
)}
...
const [booksDrawerOpen, setAccountingDrawerOpen] = useState(false);
...
const [utilityOrder, setUtilityOrder] = useState(["timeclock", "directory", "schedule", "inventory", "accounting", "prices", "team", "settings"]);
...
const preferred = ["timeclock", "directory", "schedule", "inventory", "accounting", "prices", "team", "settings"];
...
onClick: () => { closeAllDrawers(); setTeamDrawerOpen(true); },
...
<div className={`${enlargeMode ? "app-enlarged " : ""}app-shell max-w-md mx-auto min-h-screen bg-white`} style={(darkModeLoaded && enlargeLoaded) ? undefined : { visibility: "hidden" }}>
...
<button onClick={() => setLayoutEditMode((v) => !v)} aria-label="Customize navigation"
...
<div className="app-drawer absolute inset-0 bg-white overflow-y-auto max-w-md mx-auto">
...
<div className="fixed inset-0 z-20" style={{ top: "45px", bottom: BOTTOM_TAB_HEIGHT }}>
<div className="absolute inset-0 bg-black/30" onClick={() => setAccountingDrawerOpen(false)} />
<div className="app-drawer absolute inset-0 bg-white overflow-y-auto max-w-md mx-auto">
<div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 sticky top-0 bg-white z-10">
<p className="text-sm font-medium text-gray-900">Accounting</p>
<button onClick={() => setAccountingDrawerOpen(false)} aria-label="Close" className="text-gray-400">
<X size={18} />
</button>
</div>
<ExpensesView section="accounting" />
</div>
</div>
)}
{inventoryDrawerOpen && authUser && authUser.role === "owner" && (
<div className="fixed inset-0 z-20" style={{ top: "45px", bottom: BOTTOM_TAB_HEIGHT }}>
<div className="absolute inset-0 bg-black/30" onClick={() => setInventoryDrawerOpen(false)} />
<div className="app-drawer absolute inset-0 bg-white overflow-y-auto max-w-md mx-auto">
<div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 sticky top-0 bg-white z-10">
<p className="text-sm font-medium text-gray-900">Inventory</p>
<button onClick={() => setInventoryDrawerOpen(false)} aria-label="Close" className="text-gray-400"><X size={18} /></button>
</div>
<ExpensesView section="inventory" />
</div>
</div>
)}
{settingsDrawerOpen && (
<div className="fixed inset-0 z-20" style={{ top: "45px", bottom: BOTTOM_TAB_HEIGHT }}>
<div className="absolute inset-0 bg-black/30" onClick={() => setSettingsDrawerOpen(false)} />
<div className="app-drawer absolute inset-0 bg-white overflow-y-auto max-w-md mx-auto">
<div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 sticky top-0 bg-white z-10">
<p className="text-sm font-medium text-gray-900">Settings</p>
<button onClick={() => setSettingsDrawerOpen(false)} aria-label="Close" className="text-gray-400"><X size={18} /></button>
</div>
<SettingsHubView
authUser={authUser}
darkMode={darkMode}
...
<div className="fixed inset-0 z-20" style={{ top: "45px", bottom: BOTTOM_TAB_HEIGHT }}>
<div className="absolute inset-0 bg-black/30" onClick={() => setBackupDrawerOpen(false)} />
<div className="app-drawer absolute inset-0 bg-white overflow-y-auto max-w-md mx-auto">
<div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 sticky top-0 bg-white z-10">
<p className="text-sm font-medium text-gray-900">Data & Backup</p>
<button onClick={() => setBackupDrawerOpen(false)} aria-label="Close" className="text-gray-400"><X size={18} /></button>
</div>
<ExpensesView section="backup" />
</div>
</div>
)}
{bugsDrawerOpen && (
