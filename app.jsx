...
</div></section><section><h2 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--accent)" }}>Mowing</h2><div className="flex rounded-lg border border-gray-300 overflow-hidden text-sm mb-3"><button onClick={() => selectMowingType("recurring")} className="flex-1 py-2"
...
<label className="flex items-center gap-1.5 text-xs text-gray-600"><input type="checkbox" checked={completingPaidChecked} onChange={(e) => setCompletingPaidChecked(e.target.checked)}
...
function SettingsHubView({ authUser, darkMode, enlargeMode, onToggleDarkMode, onToggleEnlarge, onOpenProfile, onOpenTeamManagement, onOpenWebsite, onOpenPassword, onOpenBug, onOpenBackup, onStartTour, onLogout }) {
...
function PlanControls({ phone, name, plan, userRole, onChanged }) {
  ...
  const isPlus = !!(plan && plan.plan === "plus");
  const waived = isPlus && !!plan.fee_waived;
  const canChange = userRole === "owner" || userRole === "manager";
  async function setPlan(next) {
    setBusy(true); setMsg("");
    try {
      const res = await fetch(`${API_BASE}/api/customers/${encodeURIComponent(phone)}/plan`, { method: "POST", headers: authHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ plan: next }) });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) setMsg(d.error || "Couldn't change the plan."); else if (onChanged) await onChanged();
    } catch (e) { setMsg("Couldn't reach the server."); }
    setBusy(false);
  }
  return (
    <div className="border border-gray-200 rounded-lg p-2.5 mb-3 text-xs" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-center justify-between gap-2">
        <div>
          <span className="font-semibold text-gray-900">Plan: {isPlus ? "Plus" : "Basic"}</span>
          {isPlus && <span className="text-gray-500"> · {waived ? "$5 fee waived, 10% off visits" : "$5/mo + 10% off visits"}</span>}
          {waived && plan.waived_by && <p className="text-gray-500 mt-0.5">Waived with {plan.waived_by}'s PIN</p>}
        </div>
        {canChange && (
          <button onClick={() => setPlan(isPlus ? "basic" : "plus")} disabled={busy} className="shrink-0 rounded-full border px-3 py-1" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>
            {busy ? "…" : isPlus ? "Switch to Basic" : "Switch to Plus"}
          </button>
        )}
      </div>
      {isPlus && <button onClick={() => setWaiverOpen(true)} className="mt-2 underline" style={{ color: "var(--accent)" }}>{waived ? "Restore the $5 fee (needs a PIN)" : "Waive the $5 fee (needs a PIN)"}</button>}
      {msg && <p className="mt-1" style={{ color: "var(--warn)" }}>{msg}</p>}
      {waiverOpen && <FeeWaiverModal phone={phone} name={name} waive={!waived} onClose={() => setWaiverOpen(false)} onDone={async () => { setWaiverOpen(false); if (onChanged) await onChanged(); }} />}
    </div>
  );
}
...
notices: {
label: `Notices${teamNotices.filter((n) => !n.readAt).length ? ` (${teamNotices.filter((n) => !n.readAt).length} unread)` : ""}`, icon: BellIcon,
...
className="relative shrink-0 px-2.5 py-2.5" style={{ color: def.active ? "var(--accent)" : "var(--text-muted)" }}><Icon size={16} />
...
</div><div className="flex items-center shrink-0"><button onClick={loadCurrentWeather}
...
className="shrink-0 px-2.5 py-2.5" style={{ color: layoutEditMode ? "var(--accent)" : "var(--text-faint)" }}><Settings2 size={15} /></button></div></div>
{mode === "home" && (
<RoleDashboard
authUser={authUser}
...
<div className="fixed inset-0 z-20" style={{ top: "45px", bottom: BOTTOM_TAB_HEIGHT }}><div className="absolute inset-0 bg-black/30" onClick={() => setNoticesDrawerOpen(false)} /><div className="app-drawer absolute inset-0 bg-white overflow-y-auto max-w-md mx-auto"><div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 sticky top-0 bg-white z-10"><p className="text-sm font-medium text-gray-900">Notices</p><button onClick={() => setNoticesDrawerOpen(false)} aria-label="Close" className="text-gray-400"><X size={18} /></button></div><TeamNoticeCenter authUser={authUser} notices={teamNotices} onRefresh={loadTeamNotices} onClose={() => setNoticesDrawerOpen(false)} /></div></div>
)}
